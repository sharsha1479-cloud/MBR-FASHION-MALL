...
const getFirstAvailableSize = (variant: any) => {
  ...
  return (rows.find((item: any) => Number(item.stock || 0) > 0) || rows[0])?.size || '';
};
...
const WishlistPage = () => {
  ...
  const handleMoveToCart = async (productId: string, wishlistItemId: string, variant?: any) => {
    try {
      await addCartItem(productId, 1, { variantId: variant?.id, size: getFirstAvailableSize(variant) });
      ...
      <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="mt-6 text-xs sm:text-sm text-slate-500">
            Your wishlist is empty. <button onClick={() => navigate('/products')} className="text-maroon hover:text-maroon/80 font-semibold">Continue shopping.</button>
          </div>
        ) : (
          <div className="mt-4 sm:mt-6 space-y-3 sm:space-y-4">
            {items.map((item) => {
              ...
              const imageUrl = getProductImageUrl(variant?.images || product.images);
              return (
                <div key={item.id} className="grid gap-3 sm:gap-4 rounded-2xl sm:rounded-3xl border border-slate-200 bg-slate-50 p-3 sm:p-4 md:grid-cols-[100px_1fr_auto] md:items-center">
                  ...
                  <div className="min-w-0">
                    ...
                    {variant?.colorName && <p className="text-xs sm:text-sm text-slate-500">Color: {variant.colorName}</p>}
                    <div className="mt-2">
                      <ProductPrice product={variant || product} />
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 sm:gap-3 md:items-end">
                    ...
                    <button onClick={() => handleMoveToCart(product.id, item.id, variant)} className="rounded-full bg-maroon px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-white hover:bg-maroon/90 w-full md:w-auto">
                      Move to cart
                    </button>
